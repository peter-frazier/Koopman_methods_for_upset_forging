...
parser = argparse.ArgumentParser(description='LRAN figures')
...
<<<<<<< Updated upstream
            file_path = os.path.join(os.path.dirname(__file__), f'metrics_lran_{args.data_name}_ablation_tfac{tfac}_sfac{sfac}_gid1.0_gfwd1.0_glat1.0_geig0.0_seed0', 'train_metrics.mat')
=======
            file_path = os.path.join(os.path.dirname(__file__), f'metrics_lran_15-5PH_SS_682C_ablation_tfac{tfac}_sfac{sfac}_gid1.0_gfwd1.0_glat1.0_geig0.0_seed0', 'train_metrics.mat')
>>>>>>> Stashed changes
...
>>>>>>> Stashed changes
    ax.set_title('LRAN Data Ablation Study')

    plt.tight_layout()
    fig.savefig(os.path.join(os.path.dirname(__file__), f'LRAN {args.data_name} Data Ablation Study.png'))
...
if args.prediction:
    ...
    for i, sim in enumerate(test_sims):
        for j in range(traj_len):
            vtk_path = os.path.join(metric_path, f'{args.data_name}_test_sims', f'LRAN_{args.data_name}_test_sim{sim:03d}_step{j:03d}.vtu')
    ...
    for n in range(len(args.cells)):
        ...
        for i, s in enumerate(sims):
            ...
            plt.tight_layout()
            fig.savefig(os.path.join(args.metric_path, f'LRAN Cell {args.cells[n]}, Node {args.nodes[n]} Tracking.png'))
