...
parser = argparse.ArgumentParser(description='BLRAN figures')
...
if args.ablation:
    ...
    plt.tight_layout()
    fig.savefig(os.path.join(os.path.dirname(__file__), f'BLRAN {args.data_name} Data Ablation Study.png'))
...
if args.structure or args.eigenvalues or args.prediction:
    ...
    model      = BLRAN(n_x, n_u, args_dict['n_z'], args_dict['n_h'], args_dict['activation'], args_dict['alpha'], args_dict['init_scale'])
...
if args.prediction:
    ...
    for i, sim in enumerate(test_sims):
        for j in range(traj_len):
            vtk_path = os.path.join(metric_path, f'{args.data_name}_test_sims', f'BLRAN_{args.data_name}_test_sim{sim:03d}_step{j:03d}.vtu')
            err_path = os.path.join(metric_path, f'{args.data_name}_test_errs', f'BLRAN_{args.data_name}_errs_sim{sim:03d}_step{j:03d}.vtu')
    ...
    for n in range(len(args.cells)):
        for q in [args_dict['steps'], traj_len-1]:
            ...
            for i, s in enumerate(sims):
                ...
                plt.tight_layout()
                fig.savefig(os.path.join(metric_path, f'BLRAN Cell {args.cells[n]}, Node {args.nodes[n]} Tracking ({q} steps).png'))
