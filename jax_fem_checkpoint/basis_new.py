...
def get_elements(ele_type, load_type='edge'):
    ...
    if ele_type == 'HEX8':
        ...
        basix_ele = basix.CellType.hexahedron
        basix_face_ele = basix.CellType.quadrilateral
        gauss_order = 1  # reduced integration
        degree = 1
    elif ele_type == 'HEX27':
    ...
    # TODO: Check if this is correct.
        ...
        # Modification for pressure loads on shell elements
        # If the load is applied on the face, we need to use the quadrilateral face element
        # load_type = 'face'
        if load_type == 'edge':
...
def get_shape_vals_and_grads(ele_type, gauss_order=None):
    ...
    if ele_type == 'HEX8':
        shape_values = vals_and_grads[0, :, :, 0][[0, 1, 3, 2, 4, 5, 7, 6]]
        shape_grads_ref = onp.transpose(vals_and_grads[1:, :, :, 0], axes=(1, 2, 0))[[0, 1, 3, 2, 4, 5, 7, 6]]
        quad_points_ref = quad_points[[0, 4, 6, 2, 1, 5, 7, 3]]
    else:
        shape_values = vals_and_grads[0, :, :, 0]
        # shape_values2 = onp.transpose(vals_and_grads[0, :, :, 0], axes=(1, 2, 0))
        shape_grads_ref = onp.transpose(vals_and_grads[1:, :, :, 0], axes=(1, 2, 0))
        # jax.debug.print("shape_grads_ref: {quad_points}", quad_points=quad_points)
        quad_points_ref = quad_points#[re_order,:]

    logger.debug(f"ele_type = {ele_type}, quad_points.shape = (num_quads, dim) = {quad_points.shape}")
    return shape_values, shape_grads_ref, weights, quad_points_ref


def hex_face_shape_vals_grads(face_quad_points):
    def get_shape_vals(points):
        N1 = (1 - points[0]) * (1 - points[1]) * (1 - points[2])
        N2 = (points[0]) * (1 - points[1]) * (1 - points[2])
        N3 = (points[0]) * (points[1]) * (1 - points[2])
        N4 = (1 - points[0]) * (points[1]) * (1 - points[2])
        N5 = (1 - points[0]) * (1 - points[1]) * (points[2])
        N6 = (points[0]) * (1 - points[1]) * (points[2])
        N7 = (points[0]) * (points[1]) * (points[2])
        N8 = (1 - points[0]) * (points[1]) * (points[2])
        return np.array([N1, N2, N3, N4, N5, N6, N7, N8])

    def get_shape_grad_ref(points):
        N1 = np.array([-(1 - points[1]) * (1 - points[2]), -(1 - points[0]) * (1 - points[2]),
                       -(1 - points[1]) * (1 - points[0])])
        N2 = np.array([(1 - points[1]) * (1 - points[2]), -(points[0]) * (1 - points[2]),
                       -(1 - points[1]) * (points[0])])
        N3 = np.array([(points[1]) * (1 - points[2]), (points[0]) * (1 - points[2]),
                       -(points[1]) * (points[0])])
        N4 = np.array([-(points[1]) * (1 - points[2]), (1 - points[0]) * (1 - points[2]),
                       -(1 - points[0]) * (points[1])])
        # Change made: missing minus sign
        N5 = np.array([-(points[2]) * (1 - points[1]), -(1 - points[0]) * (points[2]),
                       (1 - points[0]) * (1 - points[1])])
        N6 = np.array([(points[2]) * (1 - points[1]), -(points[0]) * (points[2]),
                       (points[0]) * (1 - points[1])])
        N7 = np.array([(points[2]) * (points[1]), (points[0]) * (points[2]),
                       (points[0]) * (points[1])])
        N8 = np.array([-(points[2]) * (points[1]), (1 - points[0]) * (points[2]),
                       (1 - points[0]) * (points[1])])
        return np.array([N1, N2, N3, N4, N5, N6, N7, N8])

    shape_fns = jax.vmap(jax.vmap(get_shape_vals))
    grad_shape_fns = jax.vmap(jax.vmap(get_shape_grad_ref))
    shape_vals = shape_fns(face_quad_points)
    shape_grad_ref = grad_shape_fns(face_quad_points)
    return shape_vals, shape_grad_ref


def get_shape_vals_and_grads_center(ele_type, gauss_order=None):
    """TODO: Add comments
    ...
    quad_points, weights = basix.make_quadrature(basix_ele, gauss_order)
    element = basix.create_element(element_family, basix_ele, degree)
    if quad_points.shape[1]==3:
        center = onp.repeat(onp.array([[0.5, 0.5, 0.5]]), len(quad_points), axis=0)
    else:
        center = onp.repeat(onp.array([[0.5, 0.5]]), len(quad_points), axis=0)
    vals_and_grads_c = element.tabulate(1, center)[:, :, re_order, :]
    shape_values_c = vals_and_grads_c[0, :, :, 0]
    shape_grads_ref_c = onp.transpose(vals_and_grads_c[1:, :, :, 0], axes=(1, 2, 0))

    logger.debug(f"ele_type = {ele_type}, quad_points.shape = (num_quads, dim) = {quad_points.shape}")
    return shape_values_c,shape_grads_ref_c
...
def get_face_shape_vals_and_grads(ele_type, gauss_order=None):
    ...
    # physical_coords = vertices[[0, 1, 3, 2, 4, 5, 7, 6]].reshape(1, -1, 3)
    dim = len(vertices[0])
    ...
    if ele_type == 'HEX8' or ele_type == 'HEX8R':
        face_inds = reorder_inds(face_inds, [0, 4, 6, 2, 1, 5, 7, 3])
        face_inds = face_inds[[2, 1, 0, 5, 4, 3]]
        # face_shape_vals = face_shape_vals[[2, 1, 0, 5, 4, 3]][:, [0, 2, 1, 3]] # correct but not needed
        # face_shape_grads_ref = face_shape_grads_ref[[2, 1, 0, 5, 4, 3]][:, [0, 2, 1, 3]] # incorrect
        face_shape_vals_j, face_shape_grads_ref_j = hex_face_shape_vals_grads(face_quad_points)
        face_shape_vals = onp.array(face_shape_vals_j)
        face_shape_grads_ref = onp.array(face_shape_grads_ref_j)
    else:
        face_inds = reorder_inds(face_inds, re_order)
    '''
    surface_points = onp.sum(face_shape_vals[:, :, :, None] * physical_coords[:, None, :, :], axis=2)
    # print(f'Face inds reshaped:\n', face_inds)
