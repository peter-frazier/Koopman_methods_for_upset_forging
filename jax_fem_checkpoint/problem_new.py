...
from jax_fem_checkpoint.fe_new import FiniteElement #fe_new
...
class Problem:
    ...
    def __post_init__(self):
        ...
        self.fes = [FiniteElement(mesh=self.mesh[i],
                                  ...
                                  gauss_order=self.gauss_order[i] if type(self.gauss_order) == type(
                                      []) else self.gauss_order,
                                  dirichlet_bc_info=self.dirichlet_bc_info[i] if type(self.dirichlet_bc_info) == type(
        ...
        if self.p_bounds:
            b_inds = onp.array(self.location_fns[0])
            if len(b_inds.shape) == 3:
                b_inds = onp.squeeze(b_inds)
            self.boundary_inds_list = [b_inds]
        else:
        ...
        for i, boundary_inds in enumerate(self.boundary_inds_list):
            cells_list_face = [cells[boundary_inds[:, 0]] for cells in
                               self.cells_list]  # [(num_selected_faces, num_nodes), ...]
        ...
        self.cells_flat = jax.vmap(lambda *x: jax.flatten_util.ravel_pytree(x)[0])(
            *self.cells_list)  # (num_cells, num_nodes + ...)
        ...
        ## get active degree of freedom from total dof
        ###======================================
        # bnd_nodes = onp.unique(onp.array(self.fes[0].node_inds_list))
        # bnd_dofs = (onp.array(self.fes[0].node_inds_list) * self.fes[0].vec + onp.array(
        #     self.fes[0].vec_inds_list)).reshape(-1)
        # active_dof = onp.setdiff1d(onp.arange(self.num_total_dofs_all_vars), bnd_dofs)
        ...
        for boundary_inds in self.boundary_inds_list:
            ...
            if self.dim == 2:
                n_scale = onp.transpose(onp.stack(n_scale[0]), axes=(1, 0, 2)) ### # for 2D case
            else:
                n_scale = onp.transpose(onp.stack(n_scale), axes=(1, 0, 2))  ### n_scale
    ...
    def update_nanson_scale(self, sol):
        for boundary_inds in self.boundary_inds_list:
            s_shape_grads = []
            n_scale = []
            s_shape_vals = []
            for fe in self.fes:
                # (num_selected_faces, num_face_quads, num_nodes, dim), (num_selected_faces, num_face_quads)
                face_shape_grads_physical, nanson_scale = fe.get_face_shape_grads(boundary_inds, sol)
                selected_face_shape_vals = fe.face_shape_vals[
                    boundary_inds[:, 1]]  # (num_selected_faces, num_face_quads, num_nodes)
                s_shape_grads.append(face_shape_grads_physical)
                n_scale.append(nanson_scale)
    ...
    def pre_jit_fns(self):
        ...
        def get_kernel_fn_cell():
            ...
            def kernel_jac(cell_sol_flat, *args):
                ...
                return value_and_jacfwd(kernel_partial,
                                        cell_sol_flat)  # kernel(cell_sol_flat, *args), jax.jacfwd(kernel)(cell_sol_flat, *args)

            return kernel, kernel_jac

        def get_kernel_fn_face(ind):
            def kernel(cell_sol_flat, physical_surface_quad_points, face_shape_vals, face_shape_grads,
        ...
        kernel = jax.jit(jax.vmap(kernel)) #jax.jit
        kernel_jac = jax.jit(jax.vmap(kernel_jac)) #jax.jit
        self.kernel = kernel
    ...
    def compute_face(self, cells_sol_flat, np_version, jac_flag, internal_vars_surfaces):
        ...
        if jac_flag:
            ...
            for i, boundary_inds in enumerate(self.boundary_inds_list):
                ...
                input_collection = [selected_cell_sols_flat, self.physical_surface_quad_points[i],
    ...
    def compute_residual_vars(self, sol_list, internal_vars, internal_vars_surfaces):
        ...
        weak_form_face_flat = self.compute_face(cells_sol_flat, np, False,
                                                internal_vars_surfaces)  # [(num_selected_faces, num_nodes*vec + ...), ...]
