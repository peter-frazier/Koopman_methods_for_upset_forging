...
# -- Arguments -----------------------------------------------------------------
parser = argparse.ArgumentParser(description='BLRAN-LD -- Bilinear Recurrent Autoencoder with linear decoder')
...
# -- Model ---------------------------------------------------------------------
model   = BLRAN_LD(n_x, n_u, args.n_psi, args.n_h, args.activation, args.alpha, args.init_scale)
n_param = sum(p.numel() for p in model.parameters())
logger.debug(f'BLRAN-LD | n_x={n_x}  n_u={n_u}  n_h={args.n_h}  n_psi={args.n_psi}  n_z={model.n_z}  '
...
model_path = os.path.join(args.out_dir, 'model.pt') if args.out_dir else 'blran_ld_model.pt'
...
if not args.no_plot:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(args.out_dir, 'blran_ld_prediction.png'), dpi=150)
    plt.show()