...
# -- Arguments -----------------------------------------------------------------
parser = argparse.ArgumentParser(description='LRAN-LD -- LRAN with linear decoder')
...
# Architecture
parser.add_argument('--n_psi',      type=int,   default=6,
                    help='number of learned observables (total latent dim = n_x + n_psi)')
parser.add_argument('--n_h',        type=int,   default=2,
                    help='number of hidden layers')
parser.add_argument('--activation', type=str,   default='Tanh',
                    help='activation function used by network')
...
# -- Model ---------------------------------------------------------------------
model   = LRAN_LD(n_x, n_u, args.n_psi, args.n_h, args.activation, args.alpha, args.init_scale)
n_param = sum(p.numel() for p in model.parameters())
logger.debug(f'LRAN-LD | n_x={n_x}  n_u={n_u}  n_h={args.n_h}  n_psi={args.n_psi}  n_z={model.n_z}  '
             f'alpha={args.alpha}  (width={16*args.alpha})  activation={args.activation} | params={n_param:,}')
...
model_path = os.path.join(args.out_dir, 'model.pt') if args.out_dir else 'lran_ld_model.pt'
torch.save({'state_dict': model.state_dict(), 'args': vars(args),
            'scale': scale, 'n_x': n_x, 'n_u': n_u,
            'n_psi': args.n_psi, 'n_z': model.n_z}, model_path)
...
if args.out_dir:
    ...
    logger.info(f'Metrics saved -> {args.out_dir}/metrics.mat')
...
if not args.no_plot:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(args.out_dir, 'lran_ld_prediction.png'), dpi=150)
    plt.show()