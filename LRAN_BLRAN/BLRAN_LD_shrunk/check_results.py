"""
Aggregates BLRAN-LD hyperparameter sweep results and reports best hyperparameters.
...
Outputs:
    blran_ld_sweep_results.txt     -- ranked summary of all combinations
    best_results/avg_metrics.mat   -- averaged loss/error curves for the best combo
    best_results/seed{i}.mat       -- A, B, lifting weights + errors per seed
"""
...
# -- Write summary -------------------------------------------------------------
summary_path = 'blran_ld_sweep_results.txt'
with open(summary_path, 'w') as fh:
    fh.write(f'BLRAN-LD hyperparameter sweep -- {len(records)} combinations evaluated\n')
    fh.write('=' * len(_SEP) + '\n\n')
...
for seed in SEEDS:
    ...
    seed_data = {
        ...
        'B': sd['B.weight'].numpy(),
        **{k.replace('.', '_'): v.numpy()
           for k, v in sd.items() if k.startswith('lifting')},
