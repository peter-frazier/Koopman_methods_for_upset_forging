"""
BLRAN-LD hyperparameter sweep -- job script generator for OSC (SLURM).
...
def _tag(p, seed):
    return (f'npsi{p["n_psi"]}_a{p["alpha"]}_K{p["steps"]}'
            f'_lr{_fmt(p["lr"])}_wd{_fmt(p["wd"])}_bs{p["batch_size"]}'
            f'_gfwd{p["gamma_fwd"]}_glin{p["gamma_lin"]}'
            f'_geig{p["gamma_eig"]}_seed{seed}')


def result_dir(p, seed):
    return f'results_blran_ld_{_tag(p, seed)}'
...
if __name__ == '__main__':
    ...
    for p in all_combos():
        for seed in SEEDS:
            ...
            name  = f'blran_ld_{_tag(p, seed)}'
            fname = f'{name}.sh'

            sweep_args = (
                f'--dataset {DATASET} '
                f'--n_psi {p["n_psi"]} --alpha {p["alpha"]} --steps {p["steps"]} '
                f'--lr {_fmt(p["lr"])} --wd {_fmt(p["wd"])} --batch_size {p["batch_size"]} '
                f'--gamma_fwd {p["gamma_fwd"]} --gamma_lin {p["gamma_lin"]} '
                f'--gamma_eig {p["gamma_eig"]} '
