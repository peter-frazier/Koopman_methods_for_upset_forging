...
# -- Model ---------------------------------------------------------------------
model   = LRAN(n_x, n_u, args.n_z, args.n_h, args.activation, args.alpha, args.init_scale)
n_param = sum(p.numel() for p in model.parameters())
logger.debug(f'LRAN | n_x={n_x}  n_u={n_u}  n_z={args.n_z}  n_h={args.n_h}  alpha={args.alpha} '
...
if not args.no_plot:
    ...
    axes[0].set_title("theta'' = -(g/l)sintheta + u")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(args.out_dir, 'lran_prediction.png'), dpi=150)
    plt.show()